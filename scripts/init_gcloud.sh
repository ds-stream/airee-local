docker exec -it --privileged -u root airee_local-airflow-worker-1 bash -c 'echo "deb [signed-by=/usr/share/keyrings/cloud.google.gpg] http://packages.cloud.google.com/apt cloud-sdk main" |tee -a /etc/apt/sources.list.d/google-cloud-sdk.list && curl https://packages.cloud.google.com/apt/doc/apt-key.gpg |apt-key --keyring /usr/share/keyrings/cloud.google.gpg  add - && apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 467B942D3A79BD29 && apt-get update -y'
docker exec -it --privileged -u root airee_local-airflow-worker-1 bash -c 'apt-get install google-cloud-cli -y'
...
docker exec -it airee_local-airflow-worker-1 bash -c 'gcloud auth list'